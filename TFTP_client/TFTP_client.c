...
//制作data数据包
char* MakeData(short* n, FILE* f, int* datalen)
{
    char temp[512];
...
int main()
{
    ...
    {
        printf("+-----------------------------------------------+\n");
        printf("| 1.upload file           2.download file       |\n");
        ...
        if (choice == 1)
        {
            addr = getAddr("10.12.180.217", 69);
            printf("Please input the name of the file you want to upload:\n");
            char name[1000];
            int type;
            scanf("%s", name);


            printf("Please select the method for uploading the file:1.netascii     2.octet\n");
            scanf("%d", &type);
        ...
        if (choice == 2)
        {
            ...
            printf("Please enter the method of downloading the file:1.netascii  2.octet\n");
            scanf("%d", &type);
