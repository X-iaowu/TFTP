...
void init()         //初始化box
{
    unsigned short tmp = 0,Tmp = 0;
    for(int i = 0;i <= 65534; i++)
    {
        tmp = 0;
        tmp = (SBoxEn(i >> 12) << 12);
        tmp += (SBoxEn((i & 0x0f00) >> 8) << 8);
        tmp += (SBoxEn((i & 0x00f0) >> 4) << 4);
        tmp += SBoxEn(i & 0x000f);
...
unsigned short SPN (unsigned short Plaintext, unsigned int key) {
    for (int i = 1; i < 5; i++) {
        Plaintext ^= Rollkey(key, i);       //与轮密钥异或
        Plaintext = Box[Plaintext];         
    }
    //第5轮
    Plaintext = PBoxEn(Plaintext);      //第四轮解P盒
    Plaintext ^= Rollkey(key, 5);       //第五轮
    return Plaintext;
}
